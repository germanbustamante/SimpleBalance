...
class GetMonthlyTotalUseCase(
        ...
        return combine(totalFlow, expensesFlow) { total, expenses ->
            ...
            val categoryPercentages = if (total > BigDecimal.ZERO) {
                categoryTotals.mapValues { (_, categoryTotal) ->
                    (categoryTotal.divide(total, PERCENTAGE_SCALE, RoundingMode.HALF_UP) * BigDecimal(
                        PERCENTAGE_MULTIPLIER
                    ))
                        .setScale(PERCENTAGE_DISPLAY_SCALE, RoundingMode.HALF_UP)
                }
            } else {
                emptyMap()
            }
     ...
     */
    private fun calculateAverageExpenseAmount(expenses: List<Expense>): BigDecimal {
    ...
    private fun validateMonthYear(year: Int, month: Int) {
        require(month in JANUARY..DECEMBER) {
    ...
    companion object {
        ...
        private const val PERCENTAGE_DISPLAY_SCALE = 1
